...
export default function OutputFieldDrawer({
                                              isOutputDrawerVisible,
                                              setIsOutputDrawerVisible,
                                              saveOutput,
                                              editingOutput,
                                              deleteOutput
                                          }) {
    ...
    }, [editingOutput, isOutputDrawerVisible]);

    const handleSave = () => {
        if (!name.trim()) return setError({name: "Input name is required"});
        saveOutput({name, description});
        setIsOutputDrawerVisible(false);
    };
    ...
    return (
        <Drawer
            title={editingOutput ? "Update Output" : "Add New Output"}
            width={500}
            open={isOutputDrawerVisible}
            onClose={() => setIsOutputDrawerVisible(false)}
            footer={
                <div className="flex justify-between">
                    <Space>
                        <Button type="primary" icon={<SaveOutlined/>} onClick={handleSave}>
                            {editingOutput ? "Update" : "Create"}
                        </Button>
                        <Button onClick={() => setIsOutputDrawerVisible(false)}>Cancel</Button>
                    </Space>
